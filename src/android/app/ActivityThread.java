...
public final class ActivityThread
{
	...
	final HashMap<CompatibilityInfo, DisplayMetrics> mDefaultDisplayMetrics = new HashMap<CompatibilityInfo, DisplayMetrics>();
	...
	final HashMap<ProviderKey, ProviderClientRecord> mProviderMap = new HashMap<ProviderKey, ProviderClientRecord>();
	...
	static final class ReceiverData extends BroadcastReceiver.PendingResult
	{
		...
		{
			super(resultCode, resultData, resultExtras, TYPE_COMPONENT, ordered, sticky, token, sendingUser);
			this.intent = intent;
		}
	...
	private class ApplicationThread extends ApplicationThreadNative
	{
		...
		public final void scheduleLaunchActivity(Intent intent, IBinder token, int ident, ActivityInfo info, Configuration curConfig, CompatibilityInfo compatInfo, Bundle state, List<ResultInfo> pendingResults, List<Intent> pendingNewIntents, boolean notResumed, boolean isForward,
		...
		{
			requestRelaunchActivity(token, pendingResults, pendingNewIntents, configChanges, notResumed, config, true);
		}
		...
		public final void scheduleReceiver(Intent intent, ActivityInfo info, CompatibilityInfo compatInfo, int resultCode, String data, Bundle extras, boolean sync, int sendingUser)
		{
		...
		public final void bindApplication(String processName, ApplicationInfo appInfo, List<ProviderInfo> providers, ComponentName instrumentationName, String profileFile, ParcelFileDescriptor profileFd, boolean autoStopProfiler, Bundle instrumentationArgs,
				IInstrumentationWatcher instrumentationWatcher, int debugMode, boolean enableOpenGlTrace, boolean isRestrictedBackupMode, boolean persistent, Configuration config, CompatibilityInfo compatInfo, Map<String, IBinder> services, Bundle coreSettings)
		{
	...
	private static class ResourcesKey
	{
		...
		ResourcesKey(String resDir, int displayId, Configuration overrideConfiguration, float scale)
		{
			...
			hash = 31 * hash + (mOverrideConfiguration != null ? mOverrideConfiguration.hashCode() : 0);
	...
	DisplayMetrics getDisplayMetricsLocked(int displayId, CompatibilityInfo ci)
	{
		...
		else
		{
			...
			// FIXME: This would not be a problem if we kept the Display object
			// around
			// instead of using the raw display id everywhere. The Display
			// object caches
	 ...
	 * @param compInfo
	 *            the compability info. It will use the default compatibility
	 *            info when it's null.
	 */
	...
	{
		ResourcesKey key = new ResourcesKey(resDir, displayId, overrideConfiguration, compInfo.applicationScale);
		Resources r;
		synchronized (mPackages)
		{
	 ...
	 */
	Resources getTopLevelResources(String resDir, int displayId, Configuration overrideConfiguration, LoadedApk pkgInfo)
	{
		return getTopLevelResources(resDir, displayId, overrideConfiguration, pkgInfo.mCompatibilityInfo.get());
	}
	...
	private LoadedApk getPackageInfo(ApplicationInfo aInfo, CompatibilityInfo compatInfo, ClassLoader baseLoader, boolean securityViolation, boolean includeCode)
	{
		synchronized (mPackages)
		{
			...
			{
				if (localLOGV)
					Slog.v(TAG, (includeCode ? "Loading code package " : "Loading resource-only package ") + aInfo.packageName + " (in " + (mBoundApplication != null ? mBoundApplication.processName : null) + ")");
	...
	public ContextImpl getSystemContext()
	{
		synchronized (this)
		{
			if (mSystemContext == null)
			{
				...
				context.getResources().updateConfiguration(getConfiguration(), getDisplayMetricsLocked(Display.DEFAULT_DISPLAY, CompatibilityInfo.DEFAULT_COMPATIBILITY_INFO));
	...
	private Context createBaseContextForActivity(ActivityClientRecord r, final Activity activity)
	{
		...
		// For debugging purposes, if the activity's package name contains the
		// value of
		// the "debug.use-second-display" system property as a substring, then
		// show
	...
	public final ActivityClientRecord performResumeActivity(IBinder token, boolean clearHide)
	{
		...
		if (r != null && !r.activity.mFinished)
		{
			...
			try
			{
				...
				EventLog.writeEvent(LOG_ON_RESUME_CALLED, UserHandle.myUserId(), r.activity.getComponentName().getClassName());
	...
	final Bundle performPauseActivity(ActivityClientRecord r, boolean finished, boolean saveState)
	{
		...
		try
		{
			...
			r.activity.mCalled = false;
			mInstrumentation.callActivityOnPause(r.activity);
			EventLog.writeEvent(LOG_ON_PAUSE_CALLED, UserHandle.myUserId(), r.activity.getComponentName().getClassName());
			if (!r.activity.mCalled)
			{
				throw new SuperNotCalledException("Activity " + r.intent.getComponent().toShortString() + " did not call through to super.onPause()");
			}
	...
	private void updateVisibility(ActivityClientRecord r, boolean show)
	{
		...
		if (v != null)
		{
			if (show)
			{
				...
				{
					if (DEBUG_CONFIGURATION)
						Slog.v(TAG, "Updating activity vis " + r.activityInfo.name + " with new config " + r.newConfig);
					performConfigurationChanged(r.activity, r.newConfig);
					freeTextLayoutCachesIfNeeded(r.activity.mCurrentConfig.diff(r.newConfig));
					r.newConfig = null;
				}
	...
	final void applyNonDefaultDisplayMetricsToConfigurationLocked(DisplayMetrics dm, Configuration config)
	{
		...
		config.smallestScreenWidthDp = config.screenWidthDp; // assume screen
																// does not
																// rotate
	...
	final void handleActivityConfigurationChanged(IBinder token)
	{
		...
		if (DEBUG_CONFIGURATION)
			Slog.v(TAG, "Handle activity config changed: " + r.activityInfo.name);
	...
	{
		if (mCurDefaultDisplayDpi != Configuration.DENSITY_DPI_UNDEFINED && mCurDefaultDisplayDpi != DisplayMetrics.DENSITY_DEVICE && !mDensityCompatMode)
		{
			Slog.i(TAG, "Switching default density from " + DisplayMetrics.DENSITY_DEVICE + " to " + mCurDefaultDisplayDpi);
	...
	private void handleBindApplication(AppBindData data)
	{
		 ...
		 * usage on the main event loop / UI thread.
		 * 
	 ...
	 * server may be installed permanently which is indicated by setting
	 * noReleaseNeeded to true. Other remote providers are reference counted.
	 * The initial reference count for all reference counted providers is one.
	 * Providers that are not reference counted do not have a reference count
	 * (at all).
	 * 
	 * This method detects when a provider has already been installed. When this
	 * happens, it increments the reference count of the existing provider (if
	 * appropriate) and returns the existing provider. This can happen due to
	 * concurrent attempts to acquire the same provider.
	 */
	...
	public static void main(String[] args)
	{
		...
		// ==============================================================================================================
		// ==============================================================================================================
		// 创建Context上下文的实现类， 创建Application对象……
		thread.attach(false);
		// ==============================================================================================================
		// ==============================================================================================================
		...
		// ==============================================================================================================
		// ==============================================================================================================
		// 然后调用 637行的scheduleLaunchActivity方法
		// ==============================================================================================================
		// ==============================================================================================================
