 ...
 * <p>
 * For more information about creating user interface layouts, read the <a
...
public abstract class ViewGroup extends View implements ViewParent, ViewManager
{
	...
	/**
	 * A Transformation used when drawing children, to apply on the child being
	 * drawn.
	 */
	...
	/**
	 * A Transformation used to calculate a correct invalidation area when the
	 * application is autoscaled.
	 */
	 ...
	 * }
	 */
	@ViewDebug.ExportedProperty(flagMapping = { @ViewDebug.FlagToString(mask = FLAG_CLIP_CHILDREN, equals = FLAG_CLIP_CHILDREN, name = "CLIP_CHILDREN"), @ViewDebug.FlagToString(mask = FLAG_CLIP_TO_PADDING, equals = FLAG_CLIP_TO_PADDING, name = "CLIP_TO_PADDING"),
			@ViewDebug.FlagToString(mask = FLAG_PADDING_NOT_NULL, equals = FLAG_PADDING_NOT_NULL, name = "PADDING_NOT_NULL") })
	...
	/**
	 * NOTE: If you change the flags below make sure to reflect the changes the
	 * DisplayList class
	 */
	...
	// When set, this ViewGroup caches its children in a Bitmap before starting
	// a layout animation
	// Set by default
	...
	/**
	 * When set, this ViewGroup tries to always draw its children using their
	 * drawing cache.
	 */
	static final int FLAG_ALWAYS_DRAWN_WITH_CACHE = 0x4000;

	/**
	 * When set, and if FLAG_ALWAYS_DRAWN_WITH_CACHE is not set, this ViewGroup
	 * will try to draw its children with their drawing cache.
	 */
	 ...
	 */
	private static final int[] DESCENDANT_FOCUSABILITY_FLAGS = { FOCUS_BEFORE_DESCENDANTS, FOCUS_AFTER_DESCENDANTS, FOCUS_BLOCK_DESCENDANTS };
	...
	/**
	 * When set, this ViewGroup will split MotionEvents to multiple child Views
	 * when appropriate.
	 */
	private static final int FLAG_SPLIT_MOTION_EVENTS = 0x200000;

	/**
	 * When set, this ViewGroup will not dispatch onAttachedToWindow calls to
	 * children when adding new views. This is used to prevent multiple
	 * onAttached calls when a ViewGroup adds children in its own onAttached
	 * method.
	 */
	...
	/**
	 * Used to indicate that the animation drawing cache should be kept in
	 * memory.
	 */
	public static final int PERSISTENT_ANIMATION_CACHE = 0x1;

	/**
	 * Used to indicate that the scrolling drawing cache should be kept in
	 * memory.
	 */
	...
	/**
	 * This constant is a {@link #setLayoutMode(int) layoutMode}. Clip bounds
	 * are the raw values of {@link #getLeft() left}, {@link #getTop() top},
	...
	// The set of views that are currently being transitioned. This list is used
	// to track views
	// being removed that should not actually be removed from the parent yet
	// because they are
	// being animated.
	private ArrayList<View> mTransitioningViews;

	// List of children changing visibility. This is used to potentially keep
	// rendering
	// views during a transition when they otherwise would have become
	// gone/invisible
	private ArrayList<View> mVisibilityChangingChildren;

	// Indicates how many of this container's child subtrees contain transient
	// state
	 ...
	 */
	@ViewDebug.ExportedProperty(category = "focus", mapping = { @ViewDebug.IntToString(from = FOCUS_BEFORE_DESCENDANTS, to = "FOCUS_BEFORE_DESCENDANTS"), @ViewDebug.IntToString(from = FOCUS_AFTER_DESCENDANTS, to = "FOCUS_AFTER_DESCENDANTS"),
	...
	public void focusableViewAvailable(View v)
	{
		...
		// getting focus
			...
			// reason
			// to break the traversal since in that case we'd actually have to
			// find
			// the focused view and make sure it wasn't FOCUS_AFTER_DESCENDANTS
			// and
	...
	public View focusSearch(View focused, int direction)
	{
		if (isRootNamespace())
		{
			...
			// into other tabs. see LocalActivityManager and TabHost for more
			// info
	...
	/**
	 * Called when a child has requested sending an {@link AccessibilityEvent}
	 ...
	 * via calling
	 * {@link android.view.View#setAccessibilityDelegate(android.view.View.AccessibilityDelegate)}
	 * its
	...
	/**
	 * Called when a child view has changed whether or not it is tracking
	 * transient state.
	 * 
	 * @hide
	 */
	...
	/**
	 * Called when a view's visibility has changed. Notify the parent to take
	...
	protected void onChildVisibilityChanged(View child, int oldVisibility, int newVisibility)
	{
		if (mTransition != null)
		{
			...
			else
			{
				...
				{
					// Only track this on disappearing views - appearing views
					// are already visible
	...
	public boolean dispatchDragEvent(DragEvent event)
	{
		...
		case DragEvent.ACTION_DRAG_ENDED:
		{
			...
			if (mDragNotifiedChildren != null)
			{
				...
				{
					// If a child was notified about an ongoing drag, it's told
					// that it's over
			...
			// We consider drag-ended to have been handled if one of our
			// children
		...
		case DragEvent.ACTION_DRAG_LOCATION:
		{
			...
			// If we've changed apparent drag target, tell the view root which
			// view
			// we're over now [for purposes of the eventual
			// drag-recipient-changed
			// notifications to the framework] and tell the new target that the
			// drag
			// has entered its bounds. The root will see setDragFocus() calls
			// all
			// the way down to the final leaf view that is handling the LOCATION
			// event
			...
			if (mCurrentDragView != target)
			{
				...
				// If we've dragged off of a child view, send it the EXITED
				// message
				...
				// If we've dragged over a new child view, send it the ENTERED
				// message
				if (target != null)
				{
			...
			// Dispatch the actual drag location notice, localized into its
			// coordinates
			if (target != null)
			{
		 ...
		 * 
		 * DRAG_ENTERED is not dispatched downwards from ViewGroup. The reason
		 * for this is that we're about to get the corresponding LOCATION event,
		 * which we will use to determine which of our children is the new
		 * target; at that point we will push a DRAG_ENTERED down to the new
		 * target child [which may itself be a ViewGroup].
		 * 
		 * DRAG_EXITED *is* dispatched all the way down immediately: once we
		 * know the drag has left this ViewGroup, we know by definition that
		 * every contained subview is also no longer under the drag point.
		 */
		...
		if (!retval)
		{
			// Call up to the View implementation that dispatches to installed
			// listeners
	...
	// Find the frontmost child view that lies under the given point, and
	// calculate
	...
	protected boolean dispatchHoverEvent(MotionEvent event)
	{
		...
		// First check whether the view group wants to intercept the hover
		// event.
		...
		// Send events to the hovered children and build a new list of hover
		// targets until
		...
		if (!interceptHover && action != MotionEvent.ACTION_HOVER_EXIT)
		{
			...
			if (childrenCount != 0)
			{
				...
				for (int i = childrenCount - 1; i >= 0; i--)
				{
					...
					// old hover target list if the child was previously
					// hovered.
		...
		// Send exit events to all previously hovered children that are no
		// longer hovered.
		while (firstOldHoverTarget != null)
		{
			...
			else
			{
				...
				// Ignore the result because hover focus has moved to a
				// different view.
		...
		else
		{
			if (mHoveredSelf)
			{
				...
				{
					// Synthesize an exit from a move or enter.
					// Ignore the result because hover focus is moving to a
					// different view.
					if (action == MotionEvent.ACTION_HOVER_MOVE)
					{
	 ...
	 * view group or to the view group's own {@link #onHoverEvent} to allow the
	 * view group a chance to intercept the hover event. This method can also be
	 * used to watch all pointer motions that occur within the bounds of the
	 * view group even when the pointer is hovering over a child of the view
	 * group rather than over the view group itself.
	 * </p>
	 * <p>
	 * The view group can prevent its children from receiving hover events by
	 * implementing this method and returning <code>true</code> to indicate that
	 * it would like to intercept hover events. The view group must continuously
	 * return <code>true</code> from {@link #onInterceptHoverEvent} for as long
	 * as it wishes to continue intercepting hover events from its children.
	 * </p>
	 * <p>
	 * Interception preserves the invariant that at most one view can be hovered
	 * at a time by transferring hover focus from the currently hovered child to
	 * the view group or vice-versa as needed.
	 * </p>
	 * <p>
	 * If this method returns <code>true</code> and a child is already hovered,
	 * then the child view will first receive a hover exit event and then the
	 * view group itself will receive a hover enter event in
	 * {@link #onHoverEvent}. Likewise, if this method had previously returned
	 * <code>true</code> to intercept hover events and instead returns
	 * <code>false</code> while the pointer is hovering within the bounds of one
	 * of a child, then the view group will first receive a hover exit event in
	 * {@link #onHoverEvent} and then the hovered child will receive a hover
	 * enter event.
	 * </p>
	 * <p>
	...
	{
		// Send the event to the focused child or to this view group if it has
		// focus.
		if ((mPrivateFlags & (PFLAG_FOCUSED | PFLAG_HAS_BOUNDS)) == (PFLAG_FOCUSED | PFLAG_HAS_BOUNDS))
		{
			...
			if (!canceled && !intercepted)
			{
				if (actionMasked == MotionEvent.ACTION_DOWN || (split && actionMasked == MotionEvent.ACTION_POINTER_DOWN) || actionMasked == MotionEvent.ACTION_HOVER_MOVE)
				{
					...
					if (childrenCount != 0)
					{
						...
						final boolean customOrder = isChildrenDrawingOrderEnabled();
						// TODO ViewGroup中触摸事件子View的派发顺序 从后到前
						for (int i = childrenCount - 1; i >= 0; i--)
						{
							final int childIndex = customOrder ? getChildDrawingOrder(childrenCount, i) : i;
							final View child = children[childIndex];
	...
	/**
	 * Resets the cancel next up flag. Returns true if the flag was previously
	 * set.
	 */
	...
	/**
	 * Gets the touch target for specified child view. Returns null if not
	 * found.
	 */
	 ...
	 * during touch event dispatch. This behavior is enabled by default for
	 * applications that target an SDK version of
	 * {@link Build.VERSION_CODES#HONEYCOMB} or newer.
	 * 
	 * <p>
	 ...
	 * 
	 * @param split
	 *            <code>true</code> to allow MotionEvents to be split and
	 *            dispatched to multiple child views. <code>false</code> to only
	 *            allow one child view to be the target of any MotionEvent
	 *            received by this ViewGroup.
	 */
	public void setMotionEventSplittingEnabled(boolean split)
	{
		// TODO Applications really shouldn't change this setting mid-touch
		// event,
		// but perhaps this should handle that case and send ACTION_CANCELs to
		// any child views
	 ...
	 * {@link #saveHierarchyState(android.util.SparseArray)} freeze()} to only
	 * this view, not to its children. For use when overriding
	 * {@link #dispatchSaveInstanceState(android.util.SparseArray)}
	 * dispatchFreeze()} to allow subclasses to freeze their own state but not
	...
	/**
	 * Perform dispatching of a
	 * {@link #restoreHierarchyState(android.util.SparseArray)} to only this
	 * view, not to its children. For use when overriding
	...
	protected void onAnimationStart()
	{
		...
		// When this ViewGroup's animation starts, build the cache for the
		// children
	...
	protected void onAnimationEnd()
	{
		...
		// When this ViewGroup's animation ends, destroy the cache of the
		// children
		if ((mGroupFlags & FLAG_ANIMATION_CACHE) == FLAG_ANIMATION_CACHE)
		{
			mGroupFlags &= ~FLAG_CHILDREN_DRAWN_WITH_CACHE;
	...
	protected void onDebugDraw(Canvas canvas)
	{
		...
		if (getLayoutMode() == OPTICAL_BOUNDS)
		{
			for (int i = 0; i < getChildCount(); i++)
			{
				...
				drawRect(canvas, c.getLeft() + insets.left, c.getTop() + insets.top, c.getRight() - insets.right, c.getBottom() - insets.bottom, Color.RED);
			}
		}
	 ...
	 * returns i.
	 * <p>
	 * NOTE: In order for this method to be called, you must enable child
	 * ordering first by calling
	 * {@link #setChildrenDrawingOrderEnabled(boolean)}.
	 * 
	 * @param i
	...
	/**
	 * This method is used to cause children of this ViewGroup to restore or
	 ...
	 * transformations on children; this causes
	 * {@link #getChildStaticTransformation(View, android.view.animation.Transformation)}
	 * to be invoked when a child is drawn.
	 * 
	 * Any subclass overriding
	 ...
	 * 
	 * @param enabled
	 *            True to enable static transformations on children, false
	 *            otherwise.
	 * 
	 * @see #getChildStaticTransformation(View,
	...
	/**
	 * Sets <code>t</code> to be the static transformation of the child, if set,
	 * returning a boolean to indicate whether a static transform was set. The
	 * default implementation simply returns <code>false</code>; subclasses may
	 * override this method for different behavior.
	 * {@link #setStaticTransformationsEnabled(boolean)} must be set to true for
	 ...
	 * {@link #draw(android.graphics.Canvas)},
	...
	private void addViewInner(View child, int index, LayoutParams params, boolean preventRequestLayout)
	{
		...
		if (mTransition != null)
		{
			// Don't prevent other add transitions from completing, but cancel
			// remove
			// transitions to let them complete the process before we add to the
			// container
	 ...
	 *            layout. A value of <code>null</code> means no transition will
	 *            run on layout changes.
	 * @attr ref android.R.styleable#ViewGroup_animateLayoutChanges
	 */
	...
	/**
	 * Gets the LayoutTransition object for this ViewGroup. If the
	 * LayoutTransition object is not null, changes in layout which occur
	 * because of children being added to or removed from the ViewGroup will be
	 * animated according to the animations defined in that LayoutTransition
	 * object. By default, the transition object is null (so layout changes are
	 ...
	 *         changes in layout. A value of <code>null</code> means no
	 *         transition will run on layout changes.
	 */
	 ...
	 * whether the parent or child should be redrawn. Proper use of this method
	 * will include also making any appropriate {@link #requestLayout()} or
	 * {@link #invalidate()} calls. For example, callers can
	 * {@link #post(Runnable) post} a {@link Runnable} which performs a
	 * {@link #requestLayout()} on the next frame, after all detach/remove calls
	 * are finished, causing layout to be run prior to redrawing the view
	 * hierarchy.
	 * 
	 * @param child
	 ...
	 * of children so that it can be retrieved by calling
	 * {@link #getChildAt(int)}.
	 * <p>
	 * This method is intended to be lightweight and makes no assumptions about
	 * whether the parent or child should be redrawn. Proper use of this method
	 ...
	 * {@link #invalidate()} calls. For example, callers can
	 * {@link #post(Runnable) post} a {@link Runnable} which performs a
	 * {@link #requestLayout()} on the next frame, after all detach/attach calls
	 * are finished, causing layout to be run prior to redrawing the view
	 * hierarchy.
	 * <p>
	 * This method should be called only for views which were detached from
	 * their parent.
	 * 
	 * @param child
	 *            the child to attach
	 ...
	 * should only be temporary; reattachment or removal should happen within
	 * the same drawing cycle as detachment. When a view is detached, its parent
	 * is null and cannot be retrieved by a call to {@link #getChildAt(int)}.
	 * 
	 * @param child
	 *            the child to detach
	 * 
	...
	public final void invalidateChild(View child, final Rect dirty)
	{
		...
		if (attachInfo != null)
		{
			...
			// Check whether the child that requests the invalidate is fully
			// opaque
			// Views being animated or transformed are not considered opaque
			// because we may
			// be invalidating their old position and need the parent to paint
			// behind them.
			...
			do
			{
				...
				// If the parent is dirty opaque or not dirty, mark it dirty
				// with the opaque
	...
	public ViewParent invalidateChildInParent(final int[] location, final Rect dirty)
	{
		if ((mPrivateFlags & PFLAG_DRAWN) == PFLAG_DRAWN || (mPrivateFlags & PFLAG_DRAWING_CACHE_VALID) == PFLAG_DRAWING_CACHE_VALID)
		{
			...
			else
			{
				...
				{
					// in case the dirty rect extends outside the bounds of this
					// container
	...
	/**
	 * Quick invalidation method called by View.invalidateViewProperty. This
	 * doesn't set the DRAWN flags and doesn't handle the Animation logic that
	 * the default invalidation methods do; all we want to do here is schedule a
	 * traversal with the appropriate dirty rect.
	 * 
	 * @hide
	 */
		...
		if (attachInfo != null)
		{
			...
			do
			{
				if (parent instanceof ViewGroup)
				{
					...
					{
						// Layered parents should be recreated, not just
						// re-issued
						parentVG.invalidate();
						parent = null;
					}
				...
				{
					// Reached the top; this calls into the usual invalidate
					// method in
	 ...
	 * parent's coordinate system, pruning the invalidation if the parent has
	...
	{
		// It doesn't make a whole lot of sense to call this on a view that
		// isn't attached,
		// but for some simple tests it can be useful. If we don't have attach
		// info this
	 ...
	 * 
	 * When this property is disabled, the ViewGroup will use the drawing cache
	 * of its children only when asked to. It's usually the task of subclasses
	 * to tell ViewGroup when to start using the drawing cache and when to stop
	 * using it.
	 * 
	 * @param always
	 ...
	 * their drawing cache.
	 * 
	 * @return true if children should be drawn with their cache, false
	 *         otherwise
	 * 
	 * @see #setAlwaysDrawnWithCacheEnabled(boolean)
	 * @see #setChildrenDrawnWithCacheEnabled(boolean)
	 ...
	 * property is ignored when {@link #isAlwaysDrawnWithCacheEnabled()} is
	 * true. A child's drawing cache will be used only if it has been enabled.
	 * 
	 * Subclasses should call this method to start and stop using the drawing
	 * cache when they perform performance sensitive operations, like scrolling
	 * or animating.
	 * 
	 * @param enabled
	 *            true if children should be drawn with their cache, false
	 *            otherwise
	 * 
	 * @see #setAlwaysDrawnWithCacheEnabled(boolean)
	...
	/**
	 * Indicates whether the ViewGroup is drawing its children in the order
	 * defined by {@link #getChildDrawingOrder(int, int)}.
	 * 
	 * @return true if children drawing order is defined by
	...
	/**
	 * Tells the ViewGroup whether to draw its children in the order defined by
	 * the method {@link #getChildDrawingOrder(int, int)}.
	 * 
	 * @param enabled
	 *            true if the order of the children when drawing is determined
	 *            by {@link #getChildDrawingOrder(int, int)}, false otherwise
	 * 
	 * @see #isChildrenDrawingOrderEnabled()
	 * @see #getChildDrawingOrder(int, int)
	 */
	 ...
	 *         {@link #PERSISTENT_ANIMATION_CACHE},
	 *         {@link #PERSISTENT_SCROLLING_CACHE} and
	 *         {@link #PERSISTENT_ALL_CACHES}
	 */
	@ViewDebug.ExportedProperty(category = "drawing", mapping = { @ViewDebug.IntToString(from = PERSISTENT_NO_CACHE, to = "NONE"), @ViewDebug.IntToString(from = PERSISTENT_ANIMATION_CACHE, to = "ANIMATION"), @ViewDebug.IntToString(from = PERSISTENT_SCROLLING_CACHE, to = "SCROLLING"),
	...
	/**
	 * Returns the basis of alignment during layout operations on this view
	 * group: either {@link #CLIP_BOUNDS} or {@link #OPTICAL_BOUNDS}.
	 * 
	...
	/**
	 * Sets the basis of alignment during the layout of this view group. Valid
	 * values are either {@link #CLIP_BOUNDS} or {@link #OPTICAL_BOUNDS}.
	 * <p>
	...
	void finishAnimatingView(final View view, Animation animation)
	{
		...
		{
			view.onAnimationEnd();
			// Should be performed by onAnimationEnd() but this avoid an
			// infinite loop,
	...
	/**
	 * Utility function called by View during invalidation to determine whether
	 * a view that is invisible or gone should still be invalidated because it
	 * is being transitioned (and therefore still needs to be drawn).
	 */
	 ...
	 * parent. This allows animations, such as those used by
	 * {@link android.app.Fragment} and
	 * {@link android.animation.LayoutTransition} to animate the removal of
	 * views. A call to this method should always be accompanied by a later call
	 * to {@link #endViewTransition(View)}, such as after an animation on the
	 * View has finished, so that the View finally gets removed.
	 * 
	 * @param view
	 ...
	 * {@link #startViewTransition(View)}. The given View is finally removed
	 * from its parent and will no longer be displayed. Note that this method
	 * does not perform the functionality of removing a view from its parent; it
	 * just discontinues the display of a View that has previously been removed.
	 * 
	 * @return view The View object that has been removed but is being kept
	 *         around in the visible hierarchy by an earlier call to
	 *         {@link #startViewTransition(View)}.
	 */
	...
	private LayoutTransition.TransitionListener mLayoutTransitionListener = new LayoutTransition.TransitionListener()
	{
		...
		{
			// We only care about disappearing items, since we need special
			// logic to keep
	...
	/**
	 * This method is called by LayoutTransition when there are 'changing'
	 * animations that need to start after the layout/setup phase. The request
	 * is forwarded to the ViewAncestor, who starts all pending transitions
	 * prior to the drawing phase in the current traversal.
	 * 
	 * @param transition
	 ...
	 * content.
	 * 
	 * The default implementation returns true for compatibility reasons.
	 * Subclasses that do not scroll should generally override this method and
	 * return false.
	 */
	 ...
	 * laid out. See {@link android.R.styleable#ViewGroup_Layout ViewGroup
	 * Layout Attributes} for a list of all child view attributes that this
	 * class supports.
	 * 
	 * <p>
	...
	public static class LayoutParams
	{
		 ...
		 * minus the parent's padding, if any. This value is deprecated starting
		 ...
		 */
		@ViewDebug.ExportedProperty(category = "layout", mapping = { @ViewDebug.IntToString(from = MATCH_PARENT, to = "MATCH_PARENT"), @ViewDebug.IntToString(from = WRAP_CONTENT, to = "WRAP_CONTENT") })
		public int width;
		 ...
		 * Subclasses that care about layoutDirection changes should override
		...
		/**
		 * Use {@code canvas} to draw suitable debugging annotations for these
		 * LayoutParameters.
		 * 
		 * @param view
	 ...
	 * {@link android.R.styleable#ViewGroup_MarginLayout ViewGroup Margin Layout
	...
	public static class MarginLayoutParams extends ViewGroup.LayoutParams
	{
		...
		/**
		 * The start margin in pixels of the child. Call
		 * {@link ViewGroup#setLayoutParams(LayoutParams)} after reassigning a
		 * new value to this field.
		 */
		@ViewDebug.ExportedProperty(category = "layout")
		...
		/**
		 * The end margin in pixels of the child. Call
		 * {@link ViewGroup#setLayoutParams(LayoutParams)} after reassigning a
		 * new value to this field.
		 */
		@ViewDebug.ExportedProperty(category = "layout")
		...
		public MarginLayoutParams(Context c, AttributeSet attrs)
		{
			...
			else
			{
				...
				startMargin = a.getDimensionPixelSize(R.styleable.ViewGroup_MarginLayout_layout_marginStart, DEFAULT_RELATIVE);
				endMargin = a.getDimensionPixelSize(R.styleable.ViewGroup_MarginLayout_layout_marginEnd, DEFAULT_RELATIVE);
			}
		 ...
		 * {@link android.view.View#requestLayout()} needs to be done so that
		 * the new margins are taken into account. Left and right margins may be
		 * overriden by {@link android.view.View#requestLayout()} depending on
		 * layout direction.
		 * 
		 * @param left
		 ...
		 * 
		 * @attr ref
		 *       android.R.styleable#ViewGroup_MarginLayout_layout_marginLeft
		 * @attr ref android.R.styleable#ViewGroup_MarginLayout_layout_marginTop
		 * @attr ref
		 *       android.R.styleable#ViewGroup_MarginLayout_layout_marginRight
		 * @attr ref
		 *       android.R.styleable#ViewGroup_MarginLayout_layout_marginBottom
		 */
		 ...
		 * {@link android.view.View#requestLayout()} needs to be done so that
		 * the new relative margins are taken into account. Left and right
		 * margins may be overriden by {@link android.view.View#requestLayout()}
		 ...
		 * 
		 * @attr ref
		 *       android.R.styleable#ViewGroup_MarginLayout_layout_marginStart
		 * @attr ref android.R.styleable#ViewGroup_MarginLayout_layout_marginTop
		...
		/**
		 * This will be called by {@link android.view.View#requestLayout()}.
		 * Left and Right margins may be overridden depending on layout
		 * direction.
		 */
		@Override
		public void resolveLayoutDirection(int layoutDirection)
		...
		public void onDebugDraw(View view, Canvas canvas)
		{
			drawRect(canvas, view.getLeft() - leftMargin, view.getTop() - topMargin, view.getRight() + rightMargin, view.getBottom() + bottomMargin, Color.MAGENTA);
		}
	}

	/*
	 * Describes a touched view and the ids of the pointers that it has
	 * captured.
	 * 
	 * This code assumes that pointer ids are always in the range 0..31 such
	 * that it can use a bitfield to track which pointer ids are present. As it
	 * happens, the lower layers of the input dispatch pipeline also use the
	...
	private static final class TouchTarget
	{
		...
		// The combined bit mask of pointer ids for all pointers captured by the
		// target.
	...
	/**
	 * Pooled class that orderes the children of a ViewGroup from start to end
	 * based on how they are laid out and the layout direction.
	 */
	...
	/**
	 * Pooled class that holds a View and its location with respect to a
	 * specified root. This enables sorting of views based on their coordinates
	 * without recomputing the position relative to the root on every
	 * comparison.
	 */
