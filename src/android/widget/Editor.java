...
public class Editor
{
	...
	private int getErrorX()
	{
		...
		final Drawables dr = mTextView.mDrawables;
		return mTextView.getWidth() - mErrorPopup.getWidth() - mTextView.getPaddingRight() - (dr != null ? dr.mDrawableSizeRight : 0) / 2 + (int) (25 * scale + 0.5f);
	}
	...
	private int getErrorY()
	{
		 ...
		 */
		final float scale = mTextView.getResources().getDisplayMetrics().density;
		return icontop + (dr != null ? dr.mDrawableHeightRight : 0) - mTextView.getHeight() - (int) (2 * scale + 0.5f);
	}
	...
	private boolean selectCurrentWord()
	{
		...
		if (klass == InputType.TYPE_CLASS_NUMBER || klass == InputType.TYPE_CLASS_PHONE || klass == InputType.TYPE_CLASS_DATETIME || variation == InputType.TYPE_TEXT_VARIATION_URI || variation == InputType.TYPE_TEXT_VARIATION_EMAIL_ADDRESS
			|| variation == InputType.TYPE_TEXT_VARIATION_WEB_EMAIL_ADDRESS || variation == InputType.TYPE_TEXT_VARIATION_FILTER)
		{
			return mTextView.selectAllText();
		}
	...
	private void drawHardwareAccelerated(Canvas canvas, Layout layout, Path highlight, Paint highlightPaint, int cursorOffsetVertical)
	{
		...
		if (layout instanceof DynamicLayout)
		{
			...
			for (int i = 0; i < numberOfBlocks; i++)
			{
				...
				((HardwareCanvas) canvas).drawDisplayList(blockDisplayList, null, 0 /* no child clipping, our TextView parent enforces it */);
	...
	private boolean isCursorInsideSuggestionSpan()
	{
		...
		SuggestionSpan[] suggestionSpans = ((Spannable) text).getSpans(mTextView.getSelectionStart(), mTextView.getSelectionEnd(), SuggestionSpan.class);
		return (suggestionSpans.length > 0);
	}

	/**
	 * @return <code>true</code> if the cursor is inside an {@link SuggestionSpan} with {@link SuggestionSpan#FLAG_EASY_CORRECT} set.
	 */
	...
	private class EasyEditPopupWindow extends PinnedPopupWindow implements OnClickListener
	{
		...
		{
			mPopupWindow = new PopupWindow(mTextView.getContext(), null, com.android.internal.R.attr.textSelectHandleWindowStyle);
	...
	private abstract class PinnedPopupWindow implements TextViewPositionListener
	{
		...
		protected void measureContent()
		{
			...
			mContentView.measure(View.MeasureSpec.makeMeasureSpec(displayMetrics.widthPixels, View.MeasureSpec.AT_MOST), View.MeasureSpec.makeMeasureSpec(displayMetrics.heightPixels, View.MeasureSpec.AT_MOST));
		}
	...
	private class SuggestionsPopupWindow extends PinnedPopupWindow implements OnItemClickListener
	{
		...
		private class SuggestionInfo
		{
			...
			TextAppearanceSpan highlightSpan = new TextAppearanceSpan(mTextView.getContext(), android.R.style.TextAppearance_SuggestionHighlight);
		}

		private class SuggestionAdapter extends BaseAdapter
		{
			private LayoutInflater mInflater = (LayoutInflater) mTextView.getContext().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
	...
	private class SelectionActionModeCallback implements ActionMode.Callback
	{
		...
		public boolean onCreateActionMode(ActionMode mode, Menu menu)
		{
			...
			{
				menu.add(0, TextView.ID_PASTE, 0, com.android.internal.R.string.paste).setIcon(styledAttributes.getResourceId(R.styleable.SelectionModeDrawables_actionModePasteDrawable, 0)).setAlphabeticShortcut('v')
