 ...
 * 
 * @attr ref android.R.styleable#TextView_text android:text
...
public class TextView extends View implements ViewTreeObserver.OnPreDrawListener
{
	 ...
	 * see {@link KeyListener#getInputType() KeyListener.getContentType()} for important details. Calling this method will replace the current
	...
	private void setKeyListenerOnly(KeyListener input)
	{
		if (mEditor == null && input == null)
			return; // null is the default value
	 ...
	 * want a Drawable there. The Drawables must already have had {@link Drawable#setBounds} called.
	 * 
	 * @attr ref android.R.styleable#TextView_drawableLeft
	 * @attr ref android.R.styleable#TextView_drawableTop
	 * @attr ref android.R.styleable#TextView_drawableRight
	...
	/**
	 * Gets the autolink mask of the text. See {@link android.text.util.Linkify#ALL Linkify.ALL} and peers for
	 * possible values.
	 * 
	 * @attr ref android.R.styleable#TextView_autoLink
	 */
	 ...
	 * variants, so you may need to use {@link #setTypeface(Typeface, int)} to get the appearance
	...
	/**
	 * Sets whether the movement method will automatically be set to {@link LinkMovementMethod} if {@link #setAutoLinkMask} has been
	...
	/**
	 * Returns whether the movement method will automatically be set to {@link LinkMovementMethod} if {@link #setAutoLinkMask} has been
	 * set to nonzero and links are detected in {@link #setText}.
	 * The default is true.
	 * 
	 * @attr ref android.R.styleable#TextView_linksClickable
	 ...
	 * (by {@link Linkify} or otherwise) if any. You can call {@link URLSpan#getURL} on them to find where they link to
	 * or use {@link Spanned#getSpanStart} and {@link Spanned#getSpanEnd} to find the region of the text they are attached to.
	 */
	...
	/**
	 * Makes the TextView at most this many pixels tall. This option is mutually exclusive with the {@link #setMaxLines(int)} method.
	 ...
	 * To style your strings, attach android.text.style.* objects to a {@link android.text.SpannableString SpannableString}, or see the
	...
	/**
	 * Sets the text that this TextView is to display (see {@link #setText(CharSequence)}) and also sets whether it is stored
	 ...
	 * to match the given content type. If the given content type is {@link EditorInfo#TYPE_NULL} then a soft keyboard will not be displayed for this text view.
	...
	public void setRawInputType(int type)
	{
		if (type == InputType.TYPE_NULL && mEditor == null)
			return; // TYPE_NULL is the default value
		createEditorIfNeeded();
	 ...
	 * will be reported to an IME with {@link EditorInfo#actionLabel} and {@link EditorInfo#actionId} when it has focus.
	 * 
	...
	/**
	 * Set the extra input data of the text, which is the {@link EditorInfo#extras TextBoxAttribute.extras} Bundle that will be filled in when creating an input connection. The
	 * given integer is the resource ID of an XML resource holding an {@link android.R.styleable#InputExtras &lt;input-extras&gt;} XML tree.
	 * 
	...
	/**
	 * Returns the error message that was set to be displayed with {@link #setError}, or <code>null</code> if no error was set
	 ...
	 * null when any key events cause changes to the TextView's text. If the <code>error</code> is <code>null</code>, the error message and icon
	 * will be cleared.
	 */
	@android.view.RemotableViewMethod
	 ...
	 * content.
	 * Use {@link #setTextIsSelectable(boolean)} or the {@link android.R.styleable#TextView_textIsSelectable} XML attribute to make this TextView
	 ...
	 * the current input method, provided by it calling {@link InputConnection#commitCompletion
	 ...
	 * a dictionnary) from the current input method, provided by it calling {@link InputConnection#commitCorrection} InputConnection.commitCorrection()}. The default
	 ...
	 * current method, provided by it calling {@link InputConnection#performPrivateCommand
	...
	public void onRtlPropertiesChanged(int layoutDirection)
	{
		...
		{
			if (mResolvedTextAlignment == TEXT_ALIGNMENT_VIEW_START || mResolvedTextAlignment == TEXT_ALIGNMENT_VIEW_END)
			{
				mLayoutAlignment = null;
			}
		}
	...
	private Layout.Alignment getLayoutAlignment()
	{
		if (mLayoutAlignment == null)
		{
			...
			case TEXT_ALIGNMENT_VIEW_START:
				mLayoutAlignment = (getLayoutDirection() == LAYOUT_DIRECTION_RTL) ? Layout.Alignment.ALIGN_RIGHT : Layout.Alignment.ALIGN_LEFT;
				break;
			case TEXT_ALIGNMENT_VIEW_END:
				mLayoutAlignment = (getLayoutDirection() == LAYOUT_DIRECTION_RTL) ? Layout.Alignment.ALIGN_LEFT : Layout.Alignment.ALIGN_RIGHT;
				break;
	 ...
	 * not the full view width with padding. {@hide}
	 */
	 ...
	 * want to {@link #setSingleLine} or {@link #setHorizontallyScrolling} to constrain the text to a single line. Use <code>null</code> to turn off ellipsizing.
	 * If {@link #setMaxLines} has been used to set two or more lines, {@link android.text.TextUtils.TruncateAt#END} and {@link android.text.TextUtils.TruncateAt#MARQUEE}* are only supported
	...
	public void setCursorVisible(boolean visible)
	{
		if (visible && mEditor == null)
			return; // visible is the default value with no edit data
		createEditorIfNeeded();
	 ...
	 * <p>
	 * In 1.0, the {@link TextWatcher#afterTextChanged} method was erroneously not called after {@link #setText} calls. Now, doing {@link #setText} if there are any text changed listeners forces the buffer type to Editable if it would not otherwise be and does call this method.
	 */
	 ...
	 * this will be one of {@link android.R.id#selectAll}, {@link android.R.id#cut}, {@link android.R.id#copy} or {@link android.R.id#paste}.
	 * 
	 ...
	 * by the IME or by the spell checker as the user types. This is done by adding {@link SuggestionSpan}s to the text.
	 * When suggestions are enabled (default), this list of suggestions will be displayed when the
	 * user asks for them on these parts of the text. This value depends on the inputType of this
	 * TextView.
	 * The class of the input type must be {@link InputType#TYPE_CLASS_TEXT}.
	 * In addition, the type variation must be one of {@link InputType#TYPE_TEXT_VARIATION_NORMAL}, {@link InputType#TYPE_TEXT_VARIATION_EMAIL_SUBJECT}, {@link InputType#TYPE_TEXT_VARIATION_LONG_MESSAGE}, {@link InputType#TYPE_TEXT_VARIATION_SHORT_MESSAGE} or
	 ...
	 * A custom implementation can add new entries in the default menu in its {@link android.view.ActionMode.Callback#onPrepareActionMode(ActionMode, Menu)} method. The
	 * default actions can also be removed from the menu using {@link Menu#removeItem(int)} and
	 * passing {@link android.R.id#selectAll}, {@link android.R.id#cut}, {@link android.R.id#copy} or {@link android.R.id#paste} ids as parameters.
	 * Returning false from {@link android.view.ActionMode.Callback#onCreateActionMode(ActionMode, Menu)} will prevent
	 * the action mode from being started.
	 * Action click events should be handled by the custom implementation of {@link android.view.ActionMode.Callback#onActionItemClicked(ActionMode, MenuItem)}.
	 * Note that text selection mode is not started when a TextView receives focus and the {@link android.R.attr#selectAllOnFocus} flag has been set. The content is highlighted in
	...
	/**
	 * User interface state that is stored by TextView for implementing {@link View#onSaveInstanceState}.
	 */
	...
	private static class CharWrapper implements CharSequence, GetChars, GraphicsOperations
	{
		...
		public void drawTextRun(Canvas c, int start, int end, int contextStart, int contextEnd, float x, float y, int flags, Paint p)
		{
			...
			c.drawTextRun(mChars, start + mStart, count, contextStart + mStart, contextCount, x, y, flags, p);
		}
	...
	private class ChangeWatcher implements TextWatcher, SpanWatcher
	{
		...
		{
			if (DEBUG_EXTRACT)
				Log.v(LOG_TAG, "beforeTextChanged start=" + start + " before=" + before + " after=" + after + ": " + buffer);
		...
		{
			if (DEBUG_EXTRACT)
				Log.v(LOG_TAG, "onSpanChanged s=" + s + " e=" + e + " st=" + st + " en=" + en + " what=" + what + ": " + buf);
