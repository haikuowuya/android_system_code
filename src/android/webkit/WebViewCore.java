...
public final class WebViewCore
{
	...
	/*
	 * range is from 70 to 400. 0 is a special value means device-dpi. The
	 * default scale factor will be always 100. -1 means undefined. The default
	 * scale factor will be WebView.DEFAULT_SCALE_PERCENT.
	 */
	...
	// Used to determine if we should monitor the WebCore thread for
	// responsiveness.
	...
	public WebViewCore(Context context, WebViewClassic w, CallbackProxy proxy, Map<String, Object> javascriptInterfaces)
	{
		...
		synchronized (WebViewCore.class)
		{
			if (sWebCoreHandler == null)
			{
				...
				{
					// Start the singleton watchdog which will monitor the
					// WebCore thread
					// to verify it's still processing messages. Note that this
					// is the only
					// time we need to check the value as all the other public
					// methods on
					// the WebCoreThreadWatchdog are no-ops if start() is not
					// called.
		...
		// Avoid constant V8 GC when memory usage equals to working set
		// estimate.
	...
	{
		/*
		 * Initialize our private BrowserFrame class to handle all frame-related
		 * functions. We need to create a new view which in turn creates a C
		 * level FrameView and attaches it to the frame.
		 */
		...
		{
			Message.obtain(mWebViewClassic.mPrivateHandler, WebViewClassic.WEBCORE_INITIALIZED_MSG_ID, mNativeClass, 0).sendToTarget();
		}
	...
	/* package */static boolean isSupportedMediaMimeType(String mimeType)
	{
		...
		return MediaFile.isAudioFileType(fileType) || MediaFile.isVideoFileType(fileType) || MediaFile.isPlayListFileType(fileType)
	 ...
	 * @param msgLevel
	 *            the log level of this message. This is a value casted to int
	...
	private void focusNodeChanged(int nodePointer, WebKitHitTest hitTest)
	{
		if (mWebViewClassic == null)
			return;
		mWebViewClassic.mPrivateHandler.obtainMessage(WebViewClassic.FOCUS_NODE_CHANGED, nodePointer, 0, hitTest).sendToTarget();
	}
	...
	{
		/*
		 * This is WebKit's FocusDirection enum (from FocusDirection.h) enum
		 * FocusDirection { FocusDirectionNone = 0, FocusDirectionForward,
		 * FocusDirectionBackward, FocusDirectionUp, FocusDirectionDown,
		 * FocusDirectionLeft, FocusDirectionRight };
		 */
	...
	/**
	 * Notify the embedding application that the origin has exceeded it's
	 * database quota.
	 * 
	 * @param url
	 *            The URL that caused the overflow.
	...
	protected void exceededDatabaseQuota(String url, String databaseIdentifier, long quota, long estimatedDatabaseSize)
	{
		...
		mCallbackProxy.onExceededDatabaseQuota(url, databaseIdentifier, quota, estimatedDatabaseSize, getUsedQuota(), new WebStorage.QuotaUpdater()
		{
			@Override
	...
	/**
	 * Notify the embedding application that the appcache has reached or
	 * exceeded its maximum allowed storage size.
	 * 
	 * @param requiredStorage
	 *            is the amount of storage, in bytes, that would be needed in
	 *            order for the last appcache operation to succeed.
	 * @param maxSize
	 *            maximum allowed Application Cache database size, in bytes.
	 */
	protected void reachedMaxAppCacheSize(long requiredStorage, long maxSize)
	{
		mCallbackProxy.onReachedMaxAppCacheSize(requiredStorage, maxSize, new WebStorage.QuotaUpdater()
		{
			@Override
			public void updateQuota(long newQuota)
			{
	...
	{
		mCallbackProxy.onGeolocationPermissionsShowPrompt(origin, new GeolocationPermissions.Callback()
		{
			@Override
	 ...
	 * @param defaultValue
	 *            The default value in the prompt input.
	 ...
	 * 
	 * @param url
	 *            The url that is requesting the dialog.
	 * @param message
	 *            The message displayed in the dialog.
	...
	/**
	 * Notify the webview that we want to exit the video fullscreen. This is
	 * called through JNI by webcore.
	 */
	protected void exitFullscreenVideo()
	{
		if (mWebViewClassic == null)
			return;
		Message message = Message.obtain(mWebViewClassic.mPrivateHandler, WebViewClassic.EXIT_FULLSCREEN_VIDEO);
	...
	/**
	 * Redraw a portion of the picture set. The Point wh returns the width and
	 * height of the overall picture.
	 */
	private native int nativeRecordContent(int nativeClass, Point wh);

	/**
	 * Notify webkit that animations have begun (on the hardware accelerated
	 * content)
	 */
	private native void nativeNotifyAnimationStarted(int nativeClass);

	private native boolean nativeKey(int nativeClass, int keyCode, int unichar, int repeatCount, boolean isShift, boolean isAlt, boolean isSym, boolean isDown);

	private native void nativeSendListBoxChoices(int nativeClass, boolean[] choices, int size);
	...
	/*
	 * Tell webkit what its width and height are, for the purposes of
	 * layout/line-breaking. These coordinates are in document space, which is
	 * the same as View coords unless we have zoomed the document (see
	 * nativeSetZoom). textWrapWidth is used by layout to wrap column around. If
	 * viewport uses fixed size, textWrapWidth can be different from width with
	 * zooming. should this be called nativeSetViewPortSize?
	 */
	private native void nativeSetSize(int nativeClass, int width, int height, int textWrapWidth, float scale, int screenWidth, int screenHeight, int anchorX, int anchorY, boolean ignoreHeight);

	private native int nativeGetContentMinPrefWidth(int nativeClass);

	// Start: functions that deal with text editing
	private native void nativeReplaceTextfieldText(int nativeClass, int oldStart, int oldEnd, String replace, int newStart, int newEnd, int textGeneration);

	private native void passToJs(int nativeClass, int gen, String currentText, int keyCode, int keyValue, boolean down, boolean cap, boolean fn, boolean sym);

	private native void nativeSetFocusControllerActive(int nativeClass, boolean active);
	...
	private native String nativeRetrieveAnchorText(int nativeClass, int x, int y);

	private native String nativeRetrieveImageSource(int nativeClass, int x, int y);

	private native boolean nativeMouseClick(int nativeClass);

	private native int nativeHandleTouchEvent(int nativeClass, int action, int[] idArray, int[] xArray, int[] yArray, int count, int actionIndex, int metaState);
	 ...
	 * 
	 * @param nativeClass
	 *            Pointer to the C++ WebViewCore object mNativeClass
	 * @param start
	 *            Beginning of selection to delete.
	 * @param end
	 *            End of selection to delete.
	 * @param textGeneration
	 *            Text generation number when delete was pressed.
	 */
	private native void nativeDeleteSelection(int nativeClass, int start, int end, int textGeneration);
	...
	private native void nativeRegisterURLSchemeAsLocal(int nativeClass, String scheme);
	 ...
	 *            Pointer to the C++ WebViewCore object mNativeClass
	 * @param origin
	 *            The origin for which Geolocation permissions are provided.
	 * @param allow
	 *            Whether Geolocation permissions are allowed.
	 * @param remember
	 *            Whether this decision should be remembered beyond the life of
	 *            the current page.
	 */
	private native void nativeGeolocationPermissionsProvide(int nativeClass, String origin, boolean allow, boolean remember);

	/**
	 * Provide WebCore with the previously visted links from the history
	 * database
	 * 
	 * @param nativeClass
	 *            TODO
	 */
	private native void nativeProvideVisitedHistory(int nativeClass, String[] history);
	 ...
	 *            Pointer to the C++ WebViewCore object mNativeClass
	 * @param direction
	 *            The direction in which to alter the selection.
	 * @param granularity
	 *            The granularity of the selection modification.
	 * 
	 * @return The selection string.
	 */
	private native String nativeModifySelection(int nativeClass, int direction, int granularity);
				...
				sWebCoreHandler = new Handler()
				{
					...
					public void handleMessage(Message msg)
					{
						...
						case EventHub.HEARTBEAT:
							// Ping back the watchdog to let it know we're still
							// processing
							// messages.
						...
						case EventHub.TRUST_STORAGE_UPDATED:
							// post a task to network thread for updating trust
							// manager
	...
	static final String[] HandlerDebugString = { "REVEAL_SELECTION", // 96
			"", // 97
			"", // = 98
		...
		private void transferMessages()
		{
			...
			mHandler = new Handler()
			{
				...
				public void handleMessage(Message msg)
				{
					...
					if (mDestroying == true && msg.what != EventHub.DESTROY)
					{
						if (DebugFlags.WEB_VIEW_CORE)
						{
					...
					case DESTROY:
						...
						synchronized (WebViewCore.this)
						{
							...
							// Wake up the WebCore thread just in case it is
							// waiting for a
							// JavaScript dialog.
					...
					case LOAD_DATA:
						...
						if (baseUrl != null)
						{
							...
							if (i > 0)
							{
								// In 1.0, WebView.loadDataWithBaseURL() could
								// access local
								// asset files using 'file' scheme URLs as long
								// as the data is
								// valid. Later versions of WebKit have
								// tightened the
								// restriction around when pages can access such
								// local URLs.
								// To maintain compatibility with 1.0, we
								// register the scheme of
								// the baseUrl to be considered local, as long
								// as it is not
					...
					case STOP_LOADING:
						...
						// first layout done to trigger the interpreted side
						// sync
						// up with native side
					...
					case SET_SCROLL_OFFSET:
						...
						nativeSetScrollOffset(mNativeClass, msg.arg1 == 1, pt.x, pt.y);
						break;
					...
					case CLEAR_SSL_PREF_TABLE:
						// FIXME: This will not work for connections currently
						// in use, as
						// they cache the certificate responses. See
						// http://b/5324235.
					...
					case ADD_JS_INTERFACE:
						...
						mBrowserFrame.addJavascriptInterface(jsData.mObject, jsData.mInterfaceName, jsData.mRequireAnnotation);
						break;
					...
					case MODIFY_SELECTION:
						mTextSelectionChangeReason = TextSelectionData.REASON_ACCESSIBILITY_INJECTOR;
						String modifiedSelectionString = nativeModifySelection(mNativeClass, msg.arg1, msg.arg2);
						mWebViewClassic.mPrivateHandler.obtainMessage(WebViewClassic.SELECTION_STRING_CHANGED, modifiedSelectionString).sendToTarget();
						mTextSelectionChangeReason = TextSelectionData.REASON_UNKNOWN;
						break;
					...
					case SAVE_WEBARCHIVE:
						...
						mWebViewClassic.mPrivateHandler.obtainMessage(WebViewClassic.SAVE_WEBARCHIVE_FINISHED, saveMessage).sendToTarget();
						break;
					...
					case AUTOFILL_FORM:
						...
						mWebViewClassic.mPrivateHandler.obtainMessage(WebViewClassic.AUTOFILL_COMPLETE, null).sendToTarget();
						break;
					...
					case COPY_TEXT:
					{
						...
						if (copiedText != null)
						{
							mWebViewClassic.mPrivateHandler.obtainMessage(WebViewClassic.COPY_TO_CLIPBOARD, copiedText).sendToTarget();
						}
						break;
					}
					case INSERT_TEXT:
					...
					case SELECT_TEXT:
					{
						...
						nativeSelectText(mNativeClass, handleId, msg.arg1, msg.arg2);
						break;
					}
					case SELECT_WORD_AT:
					{
						...
						{
							mWebViewClassic.mPrivateHandler.obtainMessage(WebViewClassic.SHOW_CARET_HANDLE).sendToTarget();
						}
						mTextSelectionChangeReason = TextSelectionData.REASON_UNKNOWN;
						break;
					}
					...
					case FIND_ALL:
					{
						...
						Message.obtain(mWebViewClassic.mPrivateHandler, WebViewClassic.UPDATE_MATCH_COUNT, request).sendToTarget();
						break;
					}
					case FIND_NEXT:
					{
	...
	{
		mEventHub.sendMessageAtFrontOfQueue(Message.obtain(null, what, arg1, arg2, obj));
	}
	...
	{
		mEventHub.sendMessageAtFrontOfQueue(Message.obtain(null, what, obj));
	}
	...
	void destroy()
	{
		synchronized (mEventHub)
		{
			...
			// PAUSE/RESUME timers will still be processed even if they get
			// handled later
	...
	private void viewSizeChanged(WebViewClassic.ViewSizeData data)
	{
		...
		nativeSetSize(mNativeClass, width, height, textwrapWidth, scale, w, screenHeight, data.mAnchorX, data.mAnchorY, data.mIgnoreHeight);
	...
	private int calculateWindowWidth(int viewWidth)
	{
		...
		if (mSettings.getUseWideViewPort())
		{
			...
			{
				// For mobile web site.
				width = Math.round(mWebViewClassic.getViewWidth() / mWebViewClassic.getDefaultZoomScale());
			}
		}
		return width;
	}
	...
	static class DrawData
	{
		...
		Point mContentSize;
		int mMinPrefWidth;
		// only non-null if it is for the first picture set after the first
		// layout
	...
	private void webkitDraw()
	{
		...
		if (draw.mBaseLayer == 0)
		{
			...
			{
				if (DebugFlags.WEB_VIEW_CORE)
					Log.v(LOGTAG, "webkitDraw abort, resending draw message");
				mEventHub.sendMessageDelayed(Message.obtain(null, EventHub.WEBKIT_DRAW), 10);
			}
			else
			{
				if (DebugFlags.WEB_VIEW_CORE)
					Log.v(LOGTAG, "webkitDraw abort, webview paused");
			}
			return;
		}
		mLastDrawData = draw;
	...
	private void webkitDraw(DrawData draw)
	{
		if (mWebViewClassic != null)
		{
			...
			if (mSettings.getUseWideViewPort())
			{
				draw.mMinPrefWidth = Math.max(mViewportWidth == -1 ? WebViewClassic.DEFAULT_VIEWPORT_WIDTH : (mViewportWidth == 0 ? mCurrentViewWidth : mViewportWidth), nativeGetContentMinPrefWidth(mNativeClass));
			}
			...
			if (DebugFlags.WEB_VIEW_CORE)
				Log.v(LOGTAG, "webkitDraw NEW_PICTURE_MSG_ID");
			pauseWebKitDraw();
			Message.obtain(mWebViewClassic.mPrivateHandler, WebViewClassic.NEW_PICTURE_MSG_ID, draw).sendToTarget();
		}
	}
	...
	private void saveViewState(OutputStream stream, ValueCallback<Boolean> callback)
	{
		...
		DrawData draw = new DrawData();
		if (DebugFlags.WEB_VIEW_CORE)
			Log.v(LOGTAG, "saveViewState start");
		draw.mBaseLayer = nativeRecordContent(mNativeClass, draw.mContentSize);
	...
	static void reducePriority()
	{
		...
		sWebCoreHandler.sendMessageAtFrontOfQueue(sWebCoreHandler.obtainMessage(WebCoreThread.REDUCE_PRIORITY));
	}
	...
	static void resumePriority()
	{
		...
		sWebCoreHandler.removeMessages(WebCoreThread.REDUCE_PRIORITY);
		sWebCoreHandler.removeMessages(WebCoreThread.RESUME_PRIORITY);
		sWebCoreHandler.sendMessageAtFrontOfQueue(sWebCoreHandler.obtainMessage(WebCoreThread.RESUME_PRIORITY));
	}
	...
	/* package */void contentDraw()
	{
		synchronized (this)
		{
			...
			// don't update the Picture until we have an initial width and
			// finish
			// the first layout
	...
	private void contentScrollTo(int x, int y, boolean animate, boolean onlyIfImeIsShowing)
	{
		...
		if (mWebViewClassic != null)
		{
			Message msg = Message.obtain(mWebViewClassic.mPrivateHandler, WebViewClassic.SCROLL_TO_MSG_ID, animate ? 1 : 0, onlyIfImeIsShowing ? 1 : 0, new Point(x, y));
			if (mDrawIsScheduled)
			{
	...
	/*
	 * Called by JNI. The coordinates are in doc coordinates, so they need to be
	 * scaled before they can be used by the view system, which happens in
	 * WebView since it (and its thread) know the current scale factor.
	 */
	private void sendViewInvalidate(int left, int top, int right, int bottom)
	{
		if (mWebViewClassic != null)
		{
			Message.obtain(mWebViewClassic.mPrivateHandler, WebViewClassic.INVAL_RECT_MSG_ID, new Rect(left, top, right, bottom)).sendToTarget();
		}
	}
	...
	// Called by JNI
	private WebMockView getWebView()
	{
	...
	private void didFirstLayout(boolean standardLoad)
	{
		...
		mWebViewClassic.mPrivateHandler.sendEmptyMessage(WebViewClassic.HIT_TEST_RESULT);
	...
	{
		// We make bad assumptions about multiplying and dividing density by
		// 100,
	...
	private void setupViewport(boolean updateViewState)
	{
		...
		mWebViewClassic.mPrivateHandler.removeMessages(WebViewClassic.UPDATE_ZOOM_DENSITY);
		if (adjust != mWebViewClassic.getDefaultZoomScale())
		{
			Message.obtain(mWebViewClassic.mPrivateHandler, WebViewClassic.UPDATE_ZOOM_DENSITY, adjust).sendToTarget();
		}
		...
		if (mViewportWidth != 0 && !updateViewState)
		{
			...
			Message.obtain(mWebViewClassic.mPrivateHandler, WebViewClassic.UPDATE_ZOOM_RANGE, viewState).sendToTarget();
			return;
		}
		...
		else
		{
			...
			{
				WebViewClassic.ViewSizeData data = new WebViewClassic.ViewSizeData();
				// mViewScale as 0 means it is in zoom overview mode. So we
				// don't
				...
				if (tentativeScale == 0)
				{
					...
					// In viewport setup time, since no content width is known,
					// we assume
					// the windowWidth will be the content width, to get a more
					// likely
					// zoom overview scale.
					...
					{
						// In case of automatic text reflow in fixed view port
						// mode.
						mInitialViewState.mTextWrapScale = mWebViewClassic.computeReadingLevelScale(data.mScale);
					}
				}
				else
				{
				...
				// We may get a call here when mCurrentViewHeight == 0 if
				// webcore completes the
				// first layout before we sync our webview dimensions to it. In
				// that case, we
				// request the real height of the webview. This is not a perfect
				// solution as we
				// are calling a WebView method from the WebCore thread. But
				// this is preferable
				// to syncing an incorrect height.
				data.mHeight = mCurrentViewHeight == 0 ? Math.round(mWebViewClassic.getViewHeight() / data.mScale) : Math.round((float) mCurrentViewHeight * data.mWidth / viewportWidth);
	...
	private void needTouchEvents(boolean need)
	{
		if (mWebViewClassic != null)
		{
			Message.obtain(mWebViewClassic.mPrivateHandler, WebViewClassic.WEBCORE_NEED_TOUCH_EVENTS, need ? 1 : 0, 0).sendToTarget();
		}
	}
	...
	private void updateTextfield(int ptr, String text, int textGeneration)
	{
		if (mWebViewClassic != null)
		{
			Message.obtain(mWebViewClassic.mPrivateHandler, WebViewClassic.UPDATE_TEXTFIELD_TEXT_MSG_ID, ptr, textGeneration, text).sendToTarget();
		}
	}
	...
	private void updateTextSelection(int pointer, int start, int end, int textGeneration, int selectionPtr)
	{
		if (mWebViewClassic != null)
		{
			Message.obtain(mWebViewClassic.mPrivateHandler, WebViewClassic.UPDATE_TEXT_SELECTION_MSG_ID, pointer, textGeneration, createTextSelection(start, end, selectionPtr)).sendToTarget();
		}
	}
	...
	private void updateTextSizeAndScroll(int pointer, int width, int height, int scrollX, int scrollY)
	{
		if (mWebViewClassic != null)
		{
			...
			Message.obtain(mWebViewClassic.mPrivateHandler, WebViewClassic.EDIT_TEXT_SIZE_CHANGED, pointer, 0, rect).sendToTarget();
		}
	}
	...
	private void clearTextEntry()
	{
		if (mWebViewClassic == null)
			return;
		Message.obtain(mWebViewClassic.mPrivateHandler, WebViewClassic.CLEAR_TEXT_ENTRY).sendToTarget();
	}
	...
	private void initEditField(int start, int end, int selectionPtr, TextFieldInitData initData)
	{
		...
		Message.obtain(mWebViewClassic.mPrivateHandler, WebViewClassic.INIT_EDIT_FIELD, initData).sendToTarget();
		Message.obtain(mWebViewClassic.mPrivateHandler, WebViewClassic.UPDATE_TEXT_SELECTION_MSG_ID, initData.mFieldPointer, 0, createTextSelection(start, end, selectionPtr)).sendToTarget();
	}

	private native void nativeRevealSelection(int nativeClass);

	private native String nativeRequestLabel(int nativeClass, int framePtr, int nodePtr);

	/**
	 * Scroll the focused textfield to (xPercent, y) in document space
	 */
	private native void nativeScrollFocusedTextInput(int nativeClass, float xPercent, int y);

	// these must be in document space (i.e. not scaled/zoomed).
	private native void nativeSetScrollOffset(int nativeClass, boolean sendScrollEvent, int dx, int dy);

	private native void nativeSetGlobalBounds(int nativeClass, int x, int y, int w, int h);

	// called by JNI
	private void requestListBox(String[] array, int[] enabledArray, int[] selectedArray)
	{
		if (mWebViewClassic != null)
		{
	...
	// called by JNI
	private void requestListBox(String[] array, int[] enabledArray, int selection)
	{
		if (mWebViewClassic != null)
		{
	...
	private void requestKeyboard(boolean showKeyboard)
	{
		if (mWebViewClassic != null)
		{
			Message.obtain(mWebViewClassic.mPrivateHandler, WebViewClassic.REQUEST_KEYBOARD, showKeyboard ? 1 : 0, 0).sendToTarget();
		}
	}
	...
	private void setWebTextViewAutoFillable(int queryId, String preview)
	{
		if (mWebViewClassic != null)
		{
			Message.obtain(mWebViewClassic.mPrivateHandler, WebViewClassic.SET_AUTOFILLABLE, new AutoFillData(queryId, preview)).sendToTarget();
		}
	}
	...
	private void keepScreenOn(boolean screenOn)
	{
		if (mWebViewClassic != null)
		{
			Message message = mWebViewClassic.mPrivateHandler.obtainMessage(WebViewClassic.SCREEN_ON);
	...
	private void showFullScreenPlugin(ViewManager.ChildView childView, int orientation, int npp)
	{
		...
		Message message = mWebViewClassic.mPrivateHandler.obtainMessage(WebViewClassic.SHOW_FULLSCREEN);
	...
	private void hideFullScreenPlugin()
	{
		...
		mWebViewClassic.mPrivateHandler.obtainMessage(WebViewClassic.HIDE_FULLSCREEN).sendToTarget();
	}
	...
	private void showRect(int left, int top, int width, int height, int contentWidth, int contentHeight, float xPercentInDoc, float xPercentInView, float yPercentInDoc, float yPercentInView)
	{
		if (mWebViewClassic != null)
		{
			...
			Message.obtain(mWebViewClassic.mPrivateHandler, WebViewClassic.SHOW_RECT_MSG_ID, data).sendToTarget();
		}
	}
	...
	private void centerFitRect(int x, int y, int width, int height)
	{
		...
		mWebViewClassic.mPrivateHandler.obtainMessage(WebViewClassic.CENTER_FIT_RECT, new Rect(x, y, x + width, y + height)).sendToTarget();
	}
	...
	private void setScrollbarModes(int hMode, int vMode)
	{
		...
		mWebViewClassic.mPrivateHandler.obtainMessage(WebViewClassic.SET_SCROLLBAR_MODES, hMode, vMode).sendToTarget();
	}
	...
	{
		mDeviceMotionAndOrientationManager.setMockOrientation(canProvideAlpha, alpha, canProvideBeta, beta, canProvideGamma, gamma);
	}
	...
	private native WebKitHitTest nativeHitTest(int nativeClass, int x, int y, int slop, boolean moveMouse);
	 ...
	 * text differs for non-LTR text. Any text that isn't editable will be left
	 * unchanged.
	 * 
	 * @param nativeClass
	 *            The pointer to the native class (mNativeClass)
	 * @param startX
	 *            The X position of the top-left selection point.
	 * @param startY
	 *            The Y position of the top-left selection point.
	 * @param endX
	 *            The X position of the bottom-right selection point.
	 * @param endY
	 *            The Y position of the bottom-right selection point.
	 */
	private native void nativeDeleteText(int nativeClass, int startX, int startY, int endX, int endY);
	...
	/**
	 * Gets the text between two selection points. Note that the selection may
	 * differ from the WebView's selection because the algorithms for selecting
	 * text differs for non-LTR text.
	 * 
	 * @param nativeClass
	 *            The pointer to the native class (mNativeClass)
	 * @param startX
	 ...
	 *            The Y position of the bottom-right selection point.
	 */
	private native String nativeGetText(int nativeClass, int startX, int startY, int endX, int endY);

	private native void nativeSelectText(int nativeClass, int handleId, int x, int y);
